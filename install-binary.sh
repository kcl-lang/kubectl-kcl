...
: "${HELM_PLUGIN_DIR:="$HELM_HOME/plugins/kubectl-kcl"}"
...
getDownloadURL() {
  ...
  if [ "$SCRIPT_MODE" = "install" ] && [ -n "$version" ]; then
    DOWNLOAD_URL="https://github.com/$PROJECT_GH/releases/download/$version/kubectl-kcl-$OS-$ARCH.tgz"
  else
    DOWNLOAD_URL="https://github.com/$PROJECT_GH/releases/latest/download/kubectl-kcl-$OS-$ARCH.tgz"
  fi
}
...
rmTempDir() {
  if [ -d "${HELM_TMP:-/tmp/kubectl-kcl-tmp}" ]; then
    rm -rf "${HELM_TMP:-/tmp/kubectl-kcl-tmp}"
  fi
}
...
exit_trap() {
  ...
  if [ "$result" != "0" ]; then
    ...
    printf '\tFor support, go to https://github.com/KusionStack/kubectl-kcl.\n'
  fi
  exit $result
}
